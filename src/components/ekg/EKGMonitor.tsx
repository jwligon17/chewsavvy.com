...
const CORE_COLOR_FALLBACK = "#E2C793";
...
export const EKGMonitor = forwardRef<EKGMonitorHandle, EKGMonitorProps>(function EKGMonitor(
  ...
  const coreColorRef = useRef(CORE_COLOR_FALLBACK);
  ...
  useEffect(() => {
    ...
    const accent = getComputedStyle(root).getPropertyValue("--cs-accent").trim();
    if (accent) coreColorRef.current = accent;
  }, []);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = coreColorRef.current;
      ctx.lineWidth = glowLineWidth;
      ctx.shadowColor = coreColorRef.current;
